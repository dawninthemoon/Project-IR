...
public class ActionKeyInputManager : Singleton<ActionKeyInputManager>
{
    ...
    private bool sameTimeInputCheck(float deltaTime, ActionKeyInputData inputData, ActionKeyPresetData presetData)
    {
        ...
        switch(presetData._pressType)
        {
            ...
            case ActionKeyPressType.KeyPressed:
            {
                ...
                for(int i = 0; i < presetData._keyCount[(int)ControllerEx.GetInstance().getCurrentControllerType()]; ++i)
                {
                    if(ControllerEx.GetInstance().KeyPress(getKeyCode(i,presetData)) == false)
                        return false;
                }
        ...
        for(int i = 0; i < presetData._keyCount[(int)ControllerEx.GetInstance().getCurrentControllerType()]; ++i)
        {
            if(ControllerEx.GetInstance().KeyPress(getKeyCode(i,presetData)) == true)
                return false;
        }
    ...
    private bool singleInputCheck(ActionKeyPresetData presetData)
    {
        ...
        {
            case ActionKeyPressType.KeyDown:
                return ControllerEx.GetInstance().KeyDown(getKeyCode(0,presetData));
            case ActionKeyPressType.KeyPressed:
                return ControllerEx.GetInstance().KeyPress(getKeyCode(0,presetData));
            case ActionKeyPressType.KeyUp:
                return ControllerEx.GetInstance().KeyUp(getKeyCode(0,presetData));
        }
    ...
    {
        return presetData._keys[(int)ControllerEx.GetInstance().getCurrentControllerType()][index];
    }
